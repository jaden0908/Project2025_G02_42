<?php
/* ===========================================================
   Manage Customers (single-file CRUD) — Sidebar + Full Logic
...
if ($action === 'update' && $_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($errors) {
    ...
    header('Location: manage_customers.php?action=edit&id='.$id); exit;
  }
...
if ($action === 'restore' && $_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  header('Location: manage_customers.php?view=archived'); exit;
}
...
if ($action === 'edit') {
  ...
  if ($id) {
    ...
    $stmt->close();
    if (!$editRow) { $_SESSION['flash_error'] = 'Customer not found.'; header('Location: manage_customers.php'); exit; }
  }
}
    ...
    <div class="row g-3">
      ...
      <div class="col-lg-8">
        <div class="card-soft p-3">
          <form class="row g-2 mb-2" method="get" action="manage_customers.php">
            ...
            <div class="col-md-6 text-end">
              <div class="btn-group">
                <a class="btn btn-outline-secondary btn-pill<?= $view==='active'?' active':'' ?>" href="manage_customers.php?view=active">Active</a>
                <a class="btn btn-outline-secondary btn-pill<?= $view==='archived'?' active':'' ?>" href="manage_customers.php?view=archived">Archived</a>
              </div>
            ...
            <table class="table align-middle">
              ...
              <?php else: foreach ($rows as $idx => $r): ?>
                <tr>
                  ...
                  <td class="text-end">
                    <a class="btn btn-sm btn-outline-primary btn-pill" href="manage_customers.php?action=edit&id=<?= (int)$r['id'] ?>">
                      <i class="bi bi-pencil-square me-1"></i>Edit
                    </a>
                    <?php if (empty($r['deleted_at'])): ?>
                      <form class="d-inline" method="post" action="manage_customers.php?action=delete" onsubmit="return confirm('Archive this customer?');">
                        <input type="hidden" name="csrf" value="<?= e($CSRF) ?>">
                        <input type="hidden" name="id" value="<?= (int)$r['id'] ?>">
                        <button class="btn btn-sm btn-outline-danger btn-pill"><i class="bi bi-archive me-1"></i>Archive</button>
                      </form>
                    <?php else: ?>
                      <form class="d-inline" method="post" action="manage_customers.php?action=restore" onsubmit="return confirm('Restore this customer?');">
                        <input type="hidden" name="csrf" value="<?= e($CSRF) ?>">
                        <input type="hidden" name="id" value="<?= (int)$r['id'] ?>">
          ...
          <nav>
            <ul class="pagination pagination-sm justify-content-end">
              <?php
              $base = 'manage_customers.php?view='.urlencode($view).'&q='.urlencode($search).'&page=';
      ...
      <div class="col-lg-4">
        <div class="card-soft p-3">
          <?php if ($action === 'edit' && $editRow): ?>
            ...
            <form method="post" action="manage_customers.php?action=update">
              <input type="hidden" name="csrf" value="<?= e($CSRF) ?>">
              ...
              <div class="d-flex gap-2">
                ...
                <a class="btn btn-outline-secondary btn-pill" href="manage_customers.php"><i class="bi bi-x-circle me-1"></i>Cancel</a>
              </div>
            </form>
          <?php else: ?>
